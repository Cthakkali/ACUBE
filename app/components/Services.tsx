              ...
              <div className="flex flex-col gap-6 lg:translate-x-12">
                {servicesData.map((svc, i) => (
                  <div
                    ...
                    className={`w-10 h-10 rounded-full flex items-center justify-center cursor-pointer transition-colors duration-300 shadow-xs ${activeIndex === i ? 'bg-background text-primary-1' : 'bg-gray-50 text-gray-300 hover:bg-primary-1 hover:text-white'
                      }`}
                  >
                    {svc.navIcon}
                  </div>
              ...
              >
                ...
                <div className="sticky bottom-8 z-20 flex gap-2 lg:gap-3 justify-center mt-20 max-w-3xl pr-0 lg:pr-24 py-4 bg-white/80 backdrop-blur-md rounded-xl">
                  {[0, 1, 2, 3].map((dashIdx) => (
                    <div
                      key={dashIdx}
                      className={`w-12 h-[3px] rounded-full transition-colors duration-300 ${dashIdx === i ? 'bg-[#6CA89D]' : 'bg-gray-200'
                        }`}
                    ></div>
                  ))}
                </div>
